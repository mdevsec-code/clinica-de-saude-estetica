...
│   ├── finance/                  # despesas (Fase 2)
│   ├── inventory/                # estoque (Fase 2)
...
│                                  # Financeiro, Estoque, Usuários
├── components/                   # componentes de UI do site público
├── components/admin/              # gráficos, calendário, modais do painel
...
respeita papel do usuário (`ADMIN` vs `RECEPTION`):

| Rota | Tela | Descrição |
|---|---|---|
| `/admin` | Dashboard | Métricas agregadas (agendamentos, receita, ocupação) |
| `/admin/servicos` | Catálogo | CRUD de categorias e serviços |
| `/admin/agenda` | Agenda | Visão de calendário dos agendamentos, criação manual |
| `/admin/financeiro` | Financeiro | Lançamento de despesas por categoria |
...
Tenant ──┬── User (ADMIN | RECEPTION)
         ...
         ├── Expense (controle financeiro — Fase 2)
         └── InventoryItem (controle de estoque — Fase 2)
```
...
- Nenhum segredo fica no código-fonte — tudo vem de variáveis de ambiente (`.env`, nunca commitado). `JWT_SECRET` é gerado aleatoriamente, nunca deixado com valor de exemplo.
...
| `JWT_SECRET` | ✅ | Segredo para assinatura dos tokens (mín. 16 caracteres) |
| `JWT_EXPIRES_IN` | — | Validade do token (padrão `8h`) |
| `WEB_ORIGIN` | — | Origem permitida no CORS (padrão `http://localhost:5173`) |
| `ADMIN_SEED_EMAIL` | — | E-mail do admin criado pelo `npm run seed` |
| `ADMIN_SEED_PASSWORD` | — | Senha do admin criado pelo `npm run seed` |
